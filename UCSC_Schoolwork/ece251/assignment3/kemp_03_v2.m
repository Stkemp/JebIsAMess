...
% zero out data vector except for the beginning of each square pulse
for k = 1:length(a_t)
    if mod(k,sps) - 1 ~= 0
        a_t(k) = 0;
    end
end
% This is a roundabout and quick way of doing this, but I can fix it later
% if it ends up being relevant.
...
subplot(2,1,2);
plot(f_Y, 10*log10(Y));
title('SRRC Filtered Recieved Signal PSD');
xlabel('frequency (Hz)');
ylabel('dB');
